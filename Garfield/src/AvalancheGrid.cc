...
bool AvalancheGrid::SnapToGrid(const double x, const double y, const double z,
  ...
  int iY = floor((y - m_ygrid.front()) / m_yStepSize);int iZ = 0;
  if (z < 0) {
      // Field points down, electrons rise (+z). next node is front (ceil).
      iZ = std::ceil((z - m_zgrid.front()) / m_zStepSize);
  } else {
      // Field points up, electrons fall (+z). next node is backward (floor).
      iZ = std::floor((z - m_zgrid.front()) / m_zStepSize);
  }
  ...
  // TRAVA DE SEGURANÇA 2: Impede que o erro de encaixe na malha gere um passo negativo
  if (step < 0) std::cout<<"CUUU"<<std::endl;
  // if (step < 0.) step = 0.;
